...
// -- /pets/:petId/stats
// -- /pets/:petId/stats/deleted
router.get("/:deleted?", async (req, res, next) => {
	try {
		const { petId } = req.params;
		const showDeleted =
			req.params.deleted === "deleted" ? "IS NOT NULL" : "IS NULL";
        ...
        WHERE weight_stat.date_archived ${showDeleted} AND stat.pet_id = $1`,
				[petId]
			),
			pool.query(
				`
        ...
        WHERE glucose_stat.date_archived ${showDeleted} AND stat.pet_id = $1`,
				[petId]
			),
      pool.query(
        `
        ...
        WHERE heart_rate_stat.date_archived ${showDeleted} AND stat.pet_id = $1`,
        [petId]
      ),
      pool.query(
        `
        ...
        WHERE respiratory_rate_stat.date_archived ${showDeleted} AND stat.pet_id = $1`,
        [petId]
      ),
      pool.query(
        `
        ...
        WHERE other_stat.date_archived ${showDeleted} AND stat.pet_id = $1`,
        [petId]
      )
		]);
		const result = [...weights.rows, ...glucose.rows, ...heartRate.rows, ...respiratoryRate.rows, ...other.rows];
    result.sort((a, b) => b.stat_date - a.stat_date);
		res.json(result);
    ...
    //       weight_stat.stat_id,
    //       stat.description,
    //       'Weight' AS subcategory,
    //       weight_stat.id AS sub_stat_id,
    //       weight.unit,
    //       weight_stat.weight AS value,
    //       stat.stat_date,
    //       stat.date_created,
    //       stat.date_updated
    //     FROM weight_stat
    //     JOIN stat ON weight_stat.stat_id = stat.id
    //     JOIN weight ON weight.id = weight_stat.weight_id
    //     WHERE weight_stat.date_archived ${showDeleted}
    //       AND stat.pet_id = $1
    //   )
    ...
    //   (
    //     SELECT 
    //       glucose_stat.stat_id,
    //       stat.description,
    //       'Glucose' AS subcategory,
    //       glucose_stat.id AS sub_stat_id,
    //       glucose.unit,
    //       glucose_stat.glucose_level AS value,
    //       stat.stat_date,
    //       stat.date_created,
    //       stat.date_updated
    //     FROM glucose_stat
    //     JOIN stat ON glucose_stat.stat_id = stat.id
    //     JOIN glucose ON glucose.id = glucose_stat.glucose_id
    //     WHERE glucose_stat.date_archived ${showDeleted}
    //       AND stat.pet_id = $1
    //   )
    ...
    //   (
    //     SELECT 
    //       heart_rate_stat.stat_id,
    //       stat.description,
    //       'Heart Rate' AS subcategory,
    //       heart_rate_stat.id AS sub_stat_id,
    //       'bpm' AS unit,
    //       heart_rate_stat.beats_per_minute AS value,
    //       stat.stat_date,
    //       stat.date_created,
    //       stat.date_updated
    //     FROM heart_rate_stat
    //     JOIN stat ON heart_rate_stat.stat_id = stat.id
    //     WHERE heart_rate_stat.date_archived ${showDeleted}
    //       AND stat.pet_id = $1
    //   )
    ...
    //   (
    //     SELECT 
    //       respiratory_rate_stat.stat_id,
    //       stat.description,
    //       'Respiratory Rate' AS subcategory,
    //       respiratory_rate_stat.id AS sub_stat_id,
    //       'bpm' AS unit,
    //       respiratory_rate_stat.breaths_per_minute AS value,
    //       stat.stat_date,
    //       stat.date_created,
    //       stat.date_updated
    //     FROM respiratory_rate_stat
    //     JOIN stat ON respiratory_rate_stat.stat_id = stat.id
    //     WHERE respiratory_rate_stat.date_archived ${showDeleted}
    //       AND stat.pet_id = $1
    //   )
    ...
    //   (
    //     SELECT other_stat.stat_id,
    //       stat.description,
    //       'Other' AS subcategory,
    //       other_stat.id AS sub_stat_id,
    //       NULL AS unit,
    //       other_stat.note AS value,
    //       stat.stat_date,
    //       stat.date_created,
    //       stat.date_updated
    //     FROM other_stat
    //     JOIN stat ON other_stat.stat_id = stat.id
    //     WHERE other_stat.date_archived ${showDeleted}
    //       AND stat.pet_id = $1
    //   )
...
globalRouter.patch("/:id", async (req, res, next) => {
	try {
		...
		if (immutableFields.length > 0)
			throw Object.assign(
				new Error(immutableErrors + immutableFields.join(", ")),
				{ status: 400 }
			);
...
globalRouter.patch("/:id/restore", async (req, res, next) => {
	try {
		const { id } = req.params;
		const result = await pool.query(
			"UPDATE stat SET date_archived = NULL, date_updated = NOW() WHERE id = $1 AND date_archived IS NOT NULL RETURNING id, pet_id",
			[id]
		);
		if (result.rows.length === 0)
			throw Object.assign(new Error("log not found or already exists"), {
				status: 404,
			});
		res.json({
			message: `id: ${id}, Stat log restored for petId: ${result.rows[0].pet_id}`,
		});
	} catch (err) {
		console.log(err);
		next(err);
	...
	try {
		...
		if (deleteType === "soft")
			result = await pool.query(
				"UPDATE stat SET date_archived = NOW(), date_updated = NOW() WHERE id = $1 AND date_archived IS NULL RETURNING id, pet_id",
				[id]
			);
		else if (deleteType === "hard")
			result = await pool.query(
				"DELETE FROM stat WHERE id = $1 AND date_archived IS NOT NULL RETURNING id, pet_id",
				[id]
			);
		...
		res.json({
			message: `id: ${id}, Stat log ${
				deleteType === "soft" ? "soft" : "hard"
			} deleted for petId: ${result.rows[0].pet_id}`,
		});
	} catch (err) {
		console.error(err);
		next(err);
